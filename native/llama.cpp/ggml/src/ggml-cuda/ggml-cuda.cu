...
#include "ggml-cuda/btl3-avq.cuh"
#include "ggml-cuda/btl3-int4.cuh"
#include "ggml-cuda/btl3-vocab.cuh"
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    switch (dst->op) {
        ...
        case GGML_OP_FILL:
            ...
            break;
        case GGML_OP_CUSTOM:
            if (ggml_cuda_is_btl3_avq2(dst)) {
                ggml_cuda_op_btl3_avq2(ctx, dst);
            } else if (ggml_cuda_is_btl3_int4(dst)) {
                ggml_cuda_op_btl3_int4(ctx, dst);
            } else if (ggml_cuda_is_btl3_vocab_rows(dst)) {
                ggml_cuda_op_btl3_vocab_rows(ctx, dst);
            } else if (ggml_cuda_is_btl3_vocab_head(dst)) {
                ggml_cuda_op_btl3_vocab_head(ctx, dst);
            } else {
                return false;
            }
            break;
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_CUSTOM:
            ...
            return false;
#else
            return ggml_cuda_is_btl3_avq2_probe(op) ||
                ggml_cuda_is_btl3_int4_probe(op) ||
                ggml_cuda_is_btl3_vocab_rows_probe(op) ||
                ggml_cuda_is_btl3_vocab_head_probe(op) ||
                ggml_cuda_is_btl3_avq2(op) ||
                ggml_cuda_is_btl3_int4(op) ||
                ggml_cuda_is_btl3_vocab_rows(op) ||
                ggml_cuda_is_btl3_vocab_head(op);
#endif
