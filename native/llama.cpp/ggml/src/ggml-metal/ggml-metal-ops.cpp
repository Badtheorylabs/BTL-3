...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_CUSTOM:
            {
                n_fuse = ggml_metal_is_btl3_avq2(node)
                    ? ggml_metal_op_btl3_avq2(ctx, idx)
                    : ggml_metal_is_btl3_int4(node)
                        ? ggml_metal_op_btl3_int4(ctx, idx)
                        : ggml_metal_is_btl3_vocab_rows(node)
                            ? ggml_metal_op_btl3_vocab_rows(ctx, idx)
                            : ggml_metal_op_btl3_vocab_head(ctx, idx);
            } break;
        default:
            {
...
int ggml_metal_op_btl3_avq2(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    GGML_ASSERT(ggml_metal_is_btl3_avq2(op));
    ...
    struct {
        ...
        static_cast<int32_t>(op->ne[0]),
        static_cast<int32_t>(op->src[5]->ne[0]),
        static_cast<int32_t>(op->src[2]->ne[1]),
        static_cast<int32_t>(op->ne[1] * op->ne[2] * op->ne[3]),
    };
    auto pipeline = ggml_metal_library_compile_pipeline(
        ctx->lib,
        "kernel_btl3_avq2",
        "kernel_btl3_avq2",
        nullptr);
    ...
    ggml_metal_encoder_set_bytes(
        ctx->enc, &args, sizeof(args), 0);
    for (int source = 0; source < 6; ++source) {
        ggml_metal_encoder_set_buffer(
            ctx->enc,
            ggml_metal_get_buffer_id(op->src[source]),
            source + 1);
    }
    ggml_metal_encoder_set_buffer(
        ctx->enc, ggml_metal_get_buffer_id(op), 7);
    ggml_metal_encoder_set_threadgroup_memory_size(
        ctx->enc, 1280 * sizeof(float), 0);
...
int ggml_metal_op_btl3_int4(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    GGML_ASSERT(ggml_metal_is_btl3_int4(op));
    ...
    struct {
        ...
        static_cast<int32_t>(op->ne[0]),
        static_cast<int32_t>(op->src[3]->ne[0]),
        static_cast<int32_t>(op->ne[1] * op->ne[2] * op->ne[3]),
    };
    auto pipeline = ggml_metal_library_compile_pipeline(
        ctx->lib,
    ...
    ggml_metal_encoder_set_buffer(
        ctx->enc, ggml_metal_get_buffer_id(op), 5);
    ggml_metal_encoder_set_threadgroup_memory_size(
        ctx->enc, 128 * sizeof(float), 0);
    ggml_metal_encoder_dispatch_threadgroups(
        ctx->enc,
        args.rows,
        args.tokens,
...
int ggml_metal_op_btl3_vocab_rows(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    GGML_ASSERT(ggml_metal_is_btl3_vocab_rows(op));
    struct {
        int32_t vocabulary;
        int32_t columns;
        int32_t groups;
        ...
        static_cast<int32_t>(op->src[0]->ne[1]),
        static_cast<int32_t>(op->ne[0]),
        static_cast<int32_t>(op->src[2]->ne[1]),
        static_cast<int32_t>(op->src[4]->ne[0]),
        static_cast<int32_t>(op->ne[1] * op->ne[2] * op->ne[3]),
    };
    auto pipeline = ggml_metal_library_compile_pipeline(
        ctx->lib, "kernel_btl3_vocab_rows", "kernel_btl3_vocab_rows", nullptr);
    ggml_metal_encoder_set_pipeline(ctx->enc, pipeline);
    ggml_metal_encoder_set_bytes(ctx->enc, &args, sizeof(args), 0);
    for (int source = 0; source < 8; ++source) {
        ggml_metal_encoder_set_buffer(
            ctx->enc, ggml_metal_get_buffer_id(op->src[source]), source + 1);
    }
    ggml_metal_encoder_set_buffer(
        ctx->enc, ggml_metal_get_buffer_id(op), 9);
    ggml_metal_encoder_set_threadgroup_memory_size(
        ctx->enc, 1281 * sizeof(float), 0);
    ggml_metal_encoder_dispatch_threadgroups(
        ctx->enc, args.columns / 128, args.tokens, 1, 128, 1, 1);
    return 1;
}

int ggml_metal_op_btl3_vocab_head(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    GGML_ASSERT(ggml_metal_is_btl3_vocab_head(op));
    struct {
        int32_t vocabulary;
        int32_t columns;
        int32_t groups;
    ...
    auto pipeline = ggml_metal_library_compile_pipeline(
        ctx->lib, "kernel_btl3_vocab_head", "kernel_btl3_vocab_head", nullptr);
    ggml_metal_encoder_set_pipeline(ctx->enc, pipeline);
    ggml_metal_encoder_set_bytes(ctx->enc, &args, sizeof(args), 0);
    ...
    ggml_metal_encoder_set_buffer(
        ctx->enc, ggml_metal_get_buffer_id(op), 6);
    ggml_metal_encoder_set_threadgroup_memory_size(
        ctx->enc, 1280 * sizeof(float), 0);
    ggml_metal_encoder_dispatch_threadgroups(
        ctx->enc, args.vocabulary / 128, args.tokens, 1, 128, 1, 1);
    return 1;
}
