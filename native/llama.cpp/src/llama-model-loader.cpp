...
static void weight_buft_custom_noop(
        ggml_tensor *,
        int,
        int,
        void *) {
...
static std::uintptr_t packed_probe_magic(
        ggml_backend_dev_t dev,
        btl3_packed_role role) {
    if (role == btl3_packed_role::none) {
        return 0;
    }
    const ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(dev);
    const bool cuda =
        reg != nullptr && strcmp(ggml_backend_reg_name(reg), "CUDA") == 0;
    if (cuda) {
        switch (role) {
            case btl3_packed_role::avq2:
                return btl3::cuda_avq2_probe_custom_magic;
            case btl3_packed_role::int4:
                return btl3::cuda_int4_probe_custom_magic;
            case btl3_packed_role::vocab_rows:
                return btl3::cuda_vocab_rows_probe_custom_magic;
            case btl3_packed_role::vocab_head:
                return btl3::cuda_vocab_head_probe_custom_magic;
            case btl3_packed_role::none:
                return 0;
        }
    }
    return role == btl3_packed_role::avq2
        ? btl3::packed_probe_custom_magic
        : btl3::metal_packed_probe_custom_magic;
}
...
static bool weight_buft_supported(
    ...
    switch (op) {
        ...
        case GGML_OP_CUSTOM:
            {
                ggml_tensor * args[] = { w };
                const std::uintptr_t probe_magic =
                    packed_probe_magic(dev, packed_role);
                op_tensor = ggml_custom_4d(
                    ctx,
                    GGML_TYPE_F32,
                    1,
                    1,
                    ...
                    args,
                    1,
                    weight_buft_custom_noop,
                    1,
                    reinterpret_cast<void *>(probe_magic));
            } break;
        default:
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        ggml_op op;
        const bool btl_packed =
            tn.suffix != nullptr &&
            (strncmp(tn.suffix, "avq2_", 5) == 0 ||
             strncmp(tn.suffix, "btl_int4_", 9) == 0 ||
             strncmp(tn.suffix, "btl_rescued_", 12) == 0);
        btl3_packed_role packed_role = btl3_packed_role::none;
        if (tn.suffix != nullptr &&
            strncmp(tn.suffix, "btl_int4_", 9) == 0) {
            packed_role = btl3_packed_role::int4;
        } else if (tn.suffix != nullptr &&
            strncmp(tn.suffix, "btl_rescued_", 12) == 0) {
            packed_role = btl3_packed_role::vocab_rows;
        } else if (tn.suffix != nullptr &&
            strncmp(tn.suffix, "avq2_", 5) == 0) {
            packed_role = info.layer == LLM_TENSOR_LAYER_INPUT
                ? btl3_packed_role::vocab_rows
                : info.layer == LLM_TENSOR_LAYER_OUTPUT
                    ? btl3_packed_role::vocab_head
                    : btl3_packed_role::avq2;
        }
        ...
        if (!buft) {
            buft = select_weight_buft(
                hparams, t_meta, op, packed_role, buft_list);
            if (!buft) {
