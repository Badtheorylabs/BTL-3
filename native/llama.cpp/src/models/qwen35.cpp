...
void llama_model_qwen35::load_arch_hparams(llama_model_loader & ml) {
    ml.get_key("btl3.compact", btl_compact, false);
...
void llama_model_qwen35::load_arch_tensors(llama_model_loader & ml) {
    ...
    tok_embd = create_tensor(
        ...
        { n_embd, n_vocab },
        btl_compact ? TENSOR_NOT_REQUIRED : 0);
    ...
    if (output == NULL && !btl_compact) {
        output = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), { n_embd, n_vocab }, TENSOR_DUPLICATED);
    }

    auto load_vocabulary = [&](
            llama_btl3_vocabulary_weight & weight,
            llm_tensor tensor,
            bool embedding) {
        if (!btl_compact) {
            return;
        }
        const int64_t codebook_rows = 1280;
        GGML_ASSERT(n_vocab % codebook_rows == 0);
        const int64_t groups = n_vocab / codebook_rows;
        weight.codes = create_tensor(
            tn(tensor, "avq2_codes"),
            { n_embd / 4, n_vocab },
            0);
        weight.affine_weight = create_tensor(
            tn(tensor, "avq2_affine_weight"),
            { 4, 4, groups },
            0);
        weight.affine_bias = create_tensor(
            tn(tensor, "avq2_affine_bias"),
            { 4, groups },
            0);
        weight.input_signs = create_tensor(
            tn(tensor, "avq2_input_signs"),
            { n_embd },
            0);
        if (embedding) {
            const int64_t rescued_rows = 4096;
            weight.rescued_row_ids = create_tensor(
                tn(tensor, "btl_rescued_row_ids"),
                { rescued_rows },
                0);
            weight.rescued_upper_six = create_tensor(
                tn(tensor, "btl_rescued_upper_six"),
                { n_embd * 3 / 4, rescued_rows },
                0);
            weight.rescued_scales = create_tensor(
                tn(tensor, "btl_rescued_scales"),
                { rescued_rows },
                0);
        } else {
            const int64_t residual_rank = 32;
            weight.residual_left = create_tensor(
                tn(tensor, "btl_residual_left"),
                { residual_rank, n_vocab },
                0);
            weight.residual_right = create_tensor(
                tn(tensor, "btl_residual_right"),
                { n_embd, residual_rank },
                0);
        }
    };

    if (tok_embd == nullptr) {
        load_vocabulary(btl_tok_embd, LLM_TENSOR_TOKEN_EMBD, true);
    }
    if (output == nullptr) {
        load_vocabulary(btl_output, LLM_TENSOR_OUTPUT, false);
    }

    auto load_projection = [&](
            ggml_tensor *& dense,
            llama_btl3_avq2_weight & avq,
            llm_tensor tensor,
            int il,
            int64_t columns,
            int64_t rows,
        ...
        avq.lora_a = create_tensor(
            tn(tensor, "btl_lora_a", il),
            { columns, 8 },
            TENSOR_NOT_REQUIRED);
        avq.lora_b = create_tensor(
            tn(tensor, "btl_lora_b", il),
            { 8, rows },
            TENSOR_NOT_REQUIRED);
        GGML_ASSERT((avq.lora_a == nullptr) == (avq.lora_b == nullptr));
        if (dense != nullptr) {
            return;
        }

        GGML_ASSERT(columns % 4 == 0);
        avq.codes = create_tensor(
            tn(tensor, "avq2_codes", il),
            { columns / 4, rows },
            TENSOR_NOT_REQUIRED);
        if (avq.codes == nullptr) {
            avq.int4_codes = create_tensor(
                tn(tensor, "btl_int4_codes", il),
                { columns / 2, rows },
                flags);
            avq.int4_scales = create_tensor(
                tn(tensor, "btl_int4_scales", il),
                { columns / 128, rows },
                flags);
            avq.int4_zeros = create_tensor(
                tn(tensor, "btl_int4_zeros", il),
                { columns / 128, rows },
                flags);
            return;
        }
        const std::string affine_name =
            tn(tensor, "avq2_affine_weight", il).str();
        const ggml_tensor * affine_meta =
            ml.get_tensor_meta(affine_name.c_str());
        GGML_ASSERT(affine_meta != nullptr);
        const int64_t groups = affine_meta->ne[2];
        GGML_ASSERT(groups > 0 && rows % groups == 0);
        avq.affine_weight = create_tensor(
            tn(tensor, "avq2_affine_weight", il),
            { 4, 4, groups },
            flags);
        avq.affine_bias = create_tensor(
            tn(tensor, "avq2_affine_bias", il),
            { 4, groups },
            flags);
        avq.input_signs = create_tensor(
            tn(tensor, "avq2_input_signs", il),
            { columns },
            flags);
        avq.output_signs = create_tensor(
            tn(tensor, "avq2_output_signs", il),
            { rows },
            flags);
    };
    ...
    auto load_block_trunk = [&](int il, int flags) {
        ...
        if (!hparams.is_recr(il)) {
            // Attention layers
            layer.wqkv = create_tensor(
                tn(LLM_TENSOR_ATTN_QKV, "weight", il),
                { n_embd, n_embd_head_k * n_head * 2 + n_embd_k_gqa + n_embd_v_gqa },
                TENSOR_NOT_REQUIRED | TENSOR_SKIP_IF_VIRTUAL);
            if (layer.wqkv == nullptr) {
                load_projection(
                    layer.wq, layer.btl_wq, LLM_TENSOR_ATTN_Q, il,
                    n_embd, n_embd_head_k * n_head * 2, flags);
                load_projection(
                    layer.wk, layer.btl_wk, LLM_TENSOR_ATTN_K, il,
                    n_embd, n_embd_k_gqa, flags);
                load_projection(
                    layer.wv, layer.btl_wv, LLM_TENSOR_ATTN_V, il,
                    n_embd, n_embd_v_gqa, flags);
            }
            load_projection(
                layer.wo, layer.btl_wo, LLM_TENSOR_ATTN_OUT, il,
            ...
            load_projection(
                layer.wqkv, layer.btl_wqkv, LLM_TENSOR_ATTN_QKV, il,
                n_embd, key_dim * 2 + value_dim, flags);
            load_projection(
                layer.wqkv_gate, layer.btl_wqkv_gate, LLM_TENSOR_ATTN_GATE, il,
                n_embd, value_dim, flags);
            ...
            load_projection(
                layer.ssm_out, layer.btl_ssm_out, LLM_TENSOR_SSM_OUT, il,
                value_dim, n_embd, flags);
        }

        load_projection(
            layer.ffn_gate, layer.btl_ffn_gate, LLM_TENSOR_FFN_GATE, il,
            n_embd, n_ff, flags);
        load_projection(
            layer.ffn_down, layer.btl_ffn_down, LLM_TENSOR_FFN_DOWN, il,
            n_ff, n_embd, flags);
        load_projection(
            layer.ffn_up, layer.btl_ffn_up, LLM_TENSOR_FFN_UP, il,
            n_embd, n_ff, flags);
    };
...
llama_model_qwen35::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    const auto & qwen_model =
        static_cast<const llama_model_qwen35 &>(model);
    inpL = qwen_model.btl_compact
        ? build_btl_inp_embd(model.btl_tok_embd)
    ...
    if (model.output != nullptr) {
        ...
        const auto & head = model.btl_output;
        ggml_tensor * logits = btl3::build_vocab_head(
            ...
            head.input_signs,
            cur);
        ggml_tensor * residual = ggml_mul_mat(
            ctx0,
            head.residual_right,
            cur);
        residual = ggml_mul_mat(ctx0, head.residual_left, residual);
        cur = ggml_add(ctx0, logits, residual);
    }
...
ggml_tensor * llama_model_qwen35::graph::build_btl_inp_embd(
        const llama_btl3_vocabulary_weight & weight) {
    ...
    inp->embd = ggml_new_tensor_2d(
        ctx0, GGML_TYPE_F32, n_embd_inp, ubatch.n_tokens);
    cb(inp->embd, "inp_embd", -1);
    ggml_set_input(inp->embd);

    std::array<ggml_tensor *, 2> inputs;
    inputs[0] = btl3::build_vocab_get_rows(
        ctx0,
        weight.codes,
        weight.affine_weight,
        weight.affine_bias,
        weight.input_signs,
        weight.rescued_row_ids,
        weight.rescued_upper_six,
        weight.rescued_scales,
        inp->tokens);
    inputs[1] = inp->embd;
    GGML_ASSERT(ggml_are_same_shape(inputs[0], inputs[1]));
    GGML_ASSERT(ggml_are_same_stride(inputs[0], inputs[1]));

    ggml_tensor * cur = ggml_build_forward_select(
        gf, inputs.data(), inputs.size(), ubatch.token ? 0 : 1);
    res->t_inp_embd = cur;
    cb(cur, "embd", -1);
...
std::pair<ggml_tensor *, ggml_tensor *> llama_model_qwen35::graph::build_qkvz(
    ...
    ggml_tensor * qkv_mixed = build_projection(
        model.layers[il].wqkv,
        model.layers[il].btl_wqkv,
        input);
    const auto & qwen_model = static_cast<const llama_model_qwen35 &>(model);
    if (qwen_model.btl_compact) {
        const int64_t key_dim = hparams.ssm_d_state * hparams.ssm_n_group;
        const int64_t value_dim = hparams.ssm_d_inner;
        const int64_t values_per_key = hparams.ssm_dt_rank / hparams.ssm_n_group;
        const int64_t tokens = qkv_mixed->ne[1] * qkv_mixed->ne[2] * qkv_mixed->ne[3];
        ggml_tensor * qk = ggml_view_2d(
            ctx0,
            qkv_mixed,
            2 * key_dim,
            tokens,
        ...
        ggml_tensor * value = ggml_view_2d(
            ...
            tokens,
            qkv_mixed->nb[1],
            2 * key_dim * ggml_element_size(qkv_mixed));
        value = ggml_cont(ctx0, value);
        value = grouped_to_tiled(
            value,
            hparams.ssm_d_state,
            hparams.ssm_n_group,
            values_per_key);
        qkv_mixed = ggml_concat(ctx0, qk, value, 0);
    }
    qkv_mixed = ggml_reshape_3d(ctx0, qkv_mixed, qkv_mixed->ne[0], n_seq_tokens, n_seqs);
    cb(qkv_mixed, "linear_attn_qkv_mixed", il);

    ggml_tensor * z = build_projection(
        model.layers[il].wqkv_gate,
        model.layers[il].btl_wqkv_gate,
        input);
    if (qwen_model.btl_compact) {
        z = grouped_to_tiled(
            z,
            hparams.ssm_d_state,
            hparams.ssm_n_group,
            hparams.ssm_dt_rank / hparams.ssm_n_group);
    }
    cb(z, "z", il);
...
ggml_tensor * llama_model_qwen35::graph::grouped_to_tiled(
        ggml_tensor * input,
        int64_t head_dim,
        int64_t key_heads,
        int64_t values_per_key) {
    const int64_t tail = ggml_nelements(input) /
        (head_dim * key_heads * values_per_key);
    ggml_tensor * grouped = ggml_reshape_4d(
        ctx0, input, head_dim, values_per_key, key_heads, tail);
    ggml_tensor * tiled = ggml_permute(ctx0, grouped, 0, 2, 1, 3);
    tiled = ggml_cont_4d(
        ctx0, tiled, head_dim, key_heads, values_per_key, tail);
    return ggml_reshape_4d(
        ctx0, tiled, input->ne[0], input->ne[1], input->ne[2], input->ne[3]);
}

ggml_tensor * llama_model_qwen35::graph::tiled_to_grouped(
        ggml_tensor * input,
        int64_t head_dim,
        int64_t key_heads,
        int64_t values_per_key) {
    const int64_t tail = ggml_nelements(input) /
        (head_dim * key_heads * values_per_key);
    ggml_tensor * tiled = ggml_reshape_4d(
        ctx0, input, head_dim, key_heads, values_per_key, tail);
    ggml_tensor * grouped = ggml_permute(ctx0, tiled, 0, 2, 1, 3);
    grouped = ggml_cont_4d(
        ctx0, grouped, head_dim, values_per_key, key_heads, tail);
    return ggml_reshape_4d(
        ctx0, grouped, input->ne[0], input->ne[1], input->ne[2], input->ne[3]);
}

ggml_tensor * llama_model_qwen35::graph::build_projection(
        ggml_tensor * dense,
        const llama_btl3_avq2_weight & avq,
        ggml_tensor * input) {
    ggml_tensor * result;
    if (dense != nullptr) {
        result = build_lora_mm(dense, input);
    } else if (avq.int4_codes != nullptr) {
        result = btl3::build_int4_mul_mat(
            ctx0,
            avq.int4_codes,
            avq.int4_scales,
            avq.int4_zeros,
        ...
        GGML_ASSERT(avq.codes != nullptr);
        result = btl3::build_mul_mat(
            ctx0,
            avq.codes,
            avq.affine_weight,
            avq.affine_bias,
    ...
    if (avq.lora_a != nullptr) {
        ggml_tensor * correction = ggml_mul_mat(
            ctx0,
            avq.lora_b,
            ggml_mul_mat(ctx0, avq.lora_a, input));
        result = ggml_add(ctx0, result, ggml_scale(ctx0, correction, 2.0F));
    }
    return result;
}
...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn(
    ...
    ggml_tensor * Qcur_full = build_projection(
        model.layers[il].wq, model.layers[il].btl_wq, cur);
    ...
    ggml_tensor * Kcur = build_projection(
        model.layers[il].wk, model.layers[il].btl_wk, cur);
    cb(Kcur, "Kcur", il);

    ggml_tensor * Vcur = build_projection(
        model.layers[il].wv, model.layers[il].btl_wv, cur);
    cb(Vcur, "Vcur", il);
    ...
    cur = build_projection(
        model.layers[il].wo, model.layers[il].btl_wo, cur);
...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn_linear(
    ...
    cur = build_projection(
        model.layers[il].ssm_out,
        model.layers[il].btl_ssm_out,
        final_output);
...
ggml_tensor * llama_model_qwen35::graph::build_layer_ffn(ggml_tensor * cur, const int il) {
    ...
    ggml_tensor * up = build_projection(
        model.layers[il].ffn_up,
        model.layers[il].btl_ffn_up,
        cur);
    ggml_tensor * gate = build_projection(
        model.layers[il].ffn_gate,
        model.layers[il].btl_ffn_gate,
        cur);
    cur = ggml_mul(ctx0, ggml_silu(ctx0, gate), up);
    cur = build_projection(
        model.layers[il].ffn_down,
        model.layers[il].btl_ffn_down,
        cur);
    cb(cur, "ffn_out", il);
